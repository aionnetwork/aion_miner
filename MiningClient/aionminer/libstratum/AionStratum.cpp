...
void static AionMinerThread(AionMiner* miner, int size, int pos, ISolver *solver)
{
...
void AionJob::setTarget(std::string target)
{
    if (target.size() > 0) {
		...
		serverTarget = UintToArith256(uint256S("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"));
    }
}
...
void AionMiner::start()
{
	...
	for (int i = 0; i < solvers.size(); ++i) {
		...
		minerThreads[i] = std::thread(boost::bind(&AionMinerThread, this, nThreads, i, solvers[i]));
...
AionJob* AionMiner::parseJob(const Array& params)
{
    ...
    ssHeader << params[1].get_str()    //Parent Hash
    ...
    try {
        ...
        throw std::logic_error("AionMiner::parseJob(): Invalid block header parameters");
    }
...
void AionMiner::submitSolution(const EquihashSolution& solution, const std::string& jobid)
{
...
bool benchmark_solve_equihash(const ABlock& pblock, const char *tequihash_header, unsigned int tequihash_header_len, ISolver *solver)
{
	benchmark_work.lock();
