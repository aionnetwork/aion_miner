...
// hdr -> header including nonce (32 bytes) Split must be 484 (header) | 32 (nonce)
...
bool verifyEH(const char *hdr, const char *soln) {
  ...
  crypto_generichash_blake2b_update(&state, hdr, 516);
